...
# snapshot_path = os.path.join('/project/sansa/melanoma/snapshot')
snapshot_path = '/raid/mpsych/MELANOMA/snapshot/'
# snapshot_path = '/home/sanghyuk.kim001/MELANOMA/melanoma-detection-CNN/snapshot/'
...
if DB:
    ...
    modelfiles = list(itertools.chain.from_iterable([glob.glob(f'{snapshot_path}/*/*_*.pt', recursive=True)]))
    modelnames = list(map(lambda x: pathlib.Path(os.path.basename(x)), modelfiles))
...
for idx, model_name in enumerate(modelnames):
    ...
    db_path = f'/hpcstor6/scratch01/s/sanghyuk.kim001/data/melanomaDB'
